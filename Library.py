...
def BookList():
    ...
    try:
        bookFile = open('others/Books_List.txt')
        bookFile.close()
        ...
        bookListRoot.geometry('800x600')
        bookListRoot.iconbitmap('others/Icon.ico')

        CanvasBookList = Canvas(bookListRoot) 
        CanvasBookList.config(bg='#3b858c')
        CanvasBookList.pack(expand=True , fill=BOTH)
         
        bookListFrame = Frame(bookListRoot , bg='#FAEBD7' , bd=5)
        bookListFrame.place(relx=0.25 , rely=0.1 , relwidth=0.5 , relheight=0.13)
            
        bookListLabel = Label(bookListFrame , text='BOOK LIST' , bg='black' , fg='white' , font=('Times New Roman' , 18))
        ...
        with open('others/Books_List.txt') as bookListFileTreeview:
        ...
        messagebox.showerror(title='No Book' , message='No book is registered in this library')
...
def SearchBook():
    ...
    try:
        ...
        bookSearchRoot.geometry('800x600')
        bookSearchRoot.iconbitmap('others/Icon.ico')
        
        CanvasBookSearch = Canvas(bookSearchRoot)
        CanvasBookSearch.config(bg='#893b8c')
        CanvasBookSearch.pack(expand=True , fill=BOTH)

        bookSearchFrame = Frame(bookSearchRoot , bg='#FAEBD7' , bd=5)
        bookSearchFrame.place(relx=0.25 , rely=0.1 , relwidth=0.5 , relheight=0.13)
        
        bookSearchLabel = Label(bookSearchFrame , text='Search Book' , bg='black' , fg='white' , font=('Times New Roman' , 18))
        ...
        messagebox.showerror(title='No book' , message='No book registered in this library.')
...
def Search():
    ...
    if len(searchID) == 3:
        try:
            dicSearch = {}
            booksInfoSearch = getbooksinfo('others/Books_List.txt' , dicSearch)
            ...
            messagebox.showerror(title='Book not found' , message=f'No book with ID : {searchID}.')
    else:
        messagebox.showerror(title='Invalid ID' , message="Entered book ID isn't correct.")
...
def BorrowBook(): 
    ...
    try:
        ...
        bookBorrowRoot.geometry('800x600')
        bookBorrowRoot.iconbitmap('others/Icon.ico')
        
        CanvasBookBorrow = Canvas(bookBorrowRoot)
        CanvasBookBorrow.config(bg='#748c3b')
        CanvasBookBorrow.pack(expand=True , fill=BOTH)

        bookBorrowFrame = Frame(bookBorrowRoot , bg='#FAEBD7' , bd=5)
        bookBorrowFrame.place(relx=0.25 , rely=0.1 , relwidth=0.5 , relheight=0.13)
        
        bookBorrowLabel = Label(bookBorrowFrame , text='BORROW BOOK' , bg='black' , fg='white' , font=('Times New Roman' , 18))
...
def Borrow():
    ...
    if len(borrowID) == 3:
        try:
            dicBorrow = {}
            booksInfoBorrow = getbooksinfo('others/Books_List.txt' , dicBorrow)
...
def ReturnBook(): 
    ...
    try:
        ...
        bookReturnRoot.geometry('800x600')
        bookReturnRoot.iconbitmap('others/Icon.ico')
        ...
        #Heading
        bookReturnFrame = Frame(bookReturnRoot , bg='#FAEBD7' , bd=5)
...
def Return():
    ...
    if len(returnID) == 3:
        try:
            dicReturn = {}
            booksInfoReturn = getbooksinfo('others/Books_List.txt' , dicReturn)
            if booksInfoReturn[returnID][-1] == 'taken':
                new_file = []
                with open('others/Books_List.txt' , 'r') as file:
                    for line in file:
                        if line[ : 3] == returnID:
                            ...
                            new_file.append(line)
                file.close()
                with open('others/Books_List.txt' , 'w') as file:
                    for line in new_file:
                        file.writelines(line)
                file.close()
...
def AddBook():
    ...
    bookAddRoot.geometry('800x600')
    bookAddRoot.iconbitmap('others/Icon.ico')

    CanvasBookAdd = Canvas(bookAddRoot)
    CanvasBookAdd.config(bg='#683b8c')
    CanvasBookAdd.pack(expand=True , fill=BOTH)
        
    bookAddFrame = Frame(bookAddRoot , bg='#FAEBD7' , bd=5)
    bookAddFrame.place(relx=0.25 , rely=0.1 , relwidth=0.5 , relheight=0.13)

    bookAddLabel = Label(bookAddFrame , text='Add Book' , bg='black' , fg='white' , font=('Times New Roman' , 18))
...
def Add():
    ...
    if len(bookTitle) != 0:
        if len(bookAuthor) != 0:
            lst1 = []
            bookInfoList = gettitleandauthor('others/Books_List.txt' , lst1)
            ...
            if bookInfo in bookInfoList:
                ...
                bookID = getlastid('others/Books_List.txt')
                if 99 > int(bookID) >= 9:
                ...
                bookList = open('others/Books_List.txt' , 'a')
...
def DeleteBook(): 
    ...
    try:
        ...
        bookDeleteRoot.geometry('800x600')
        bookDeleteRoot.iconbitmap('others/Icon.ico')
        ...
        bookDeleteFrame = Frame(bookDeleteRoot , bg='#FAEBD7' , bd=5)
...
def Delete():
    ...
    if len(deleteID) == 3:
        try:
            dicDelete = {}
            booksInfoDelete = getbooksinfo('others/Books_List.txt' , dicDelete)
            if deleteID in booksInfoDelete.keys():
                ...
                messagebox.showerror(title='The book was not found' , message=f'No book with ID : {deleteID}.')
        except KeyError:
            messagebox.showerror(title='The book was not found' , message=f'No book with ID : {deleteID}.')
    else:
        messagebox.showerror(title='Invalid ID' , message='Entered book ID is not correct.')
...
library_app.maxsize(width=1400 , height=800)
library_app.geometry('800x600')
library_app.iconbitmap('others/Icon.ico')

backgroundImage =Image.open('others/background.jpg')
[imageSizeWidth , imageSizeHeight] = backgroundImage.size

newImageSizeWidth = int(imageSizeWidth*0.32)
newImageSizeHeight = int(imageSizeHeight*0.3)
...
welcomeLabel = Label(welcomeFrame, text='LIBRARY MANAGEMENT SYSTEM' , bg='white' , fg='black' , font=('Times New Roman' , 22))
welcomeLabel.place(relx=0 , rely=0 , relwidth=1 , relheight=1)

btnBookList = Button(library_app , text='BOOK LIST' , bg='white' , fg='black' , font=fonts , command=BookList)
btnBookList.place(relx=0.28 , rely=0.25 , relwidth=0.45 , relheight=0.1)

btnBookSearch = Button(library_app , text='SEARCH BOOK' , bg='white' , fg='black' , font=fonts , command=SearchBook)
btnBookSearch.place(relx=0.28 , rely=0.35 , relwidth=0.45 , relheight=0.1)

btnBookBorrow = Button(library_app , text='BORROW BOOK' , bg='white' , fg='black' , font=fonts , command=BorrowBook)
btnBookBorrow.place(relx=0.28 , rely=0.45 , relwidth=0.45 , relheight=0.1)
    
btnBookReturn = Button(library_app , text='RETURN BOOK' , bg='white' , fg='black' , font=fonts , command=ReturnBook)
btnBookReturn.place(relx=0.28 , rely=0.55 , relwidth=0.45 , relheight=0.1)

btnBookAdd = Button(library_app , text='ADD BOOK' , bg='white' , fg='black' , font=fonts , command=AddBook)
btnBookAdd.place(relx=0.28 , rely=0.65 , relwidth=0.45 , relheight=0.1)

btnBookDelete = Button(library_app , text='DELETE BOOK' , bg='white' , fg='black' , font=fonts , command=DeleteBook)
btnBookDelete.place(relx=0.28 , rely=0.75 , relwidth=0.45 , relheight=0.1)

btnQuit = Button(library_app , text='EXIT' , bg='white' , fg='black' , font=fonts , command=library_app.destroy)
